...
KEY0:			LDR R1, =RUN 				// Toggles value of run variable
				LDR R2, [R1]
				MVN R2, R2
				AND R2, #1
				STR R2, [R1]
...
KEY1_OR_2:		LDR R1, =0xFF202020			// Doubles or halves rate that count is increased
				MOV R5, #0b1000				// Stop signal
				STR R5, [R1, #4]
				...
				ORR R3, R2
				CMP R4, #0b0100
				LSLEQ R3, #1				// Doubles timer -> halves count rate of increase 
				LSRNE R3, #1				// Halves timer -> doubles count rate of increase
				
				STRH R3, [R1, #8]			// Stores new value
				LSR R3, #16
				STRH R3, [R1, #0xC]
				MOV R2, #0b111
				STR R2, [R1, #4]			// Restarts timer
				B END_KEY_ISR
								
END_KEY_ISR:	MOV	R2, #0xF
